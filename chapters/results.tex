...
The intuition behind these results are that with hard attention the labeller only
...
Additionally non-top scores do not reflect alternatives but are rather only a noise,
...
Similarly as in single language configuration we have tested the impact of \emph{tagger}
...
increasing the size changes the result only slightly (and are actually worse).
...
will assign the right POS tag - the tree becomes valid parse. For an example see Figure~\ref{fig:pos_bad_good}.
...
\todotext{Podsumować wyniki polsko-polskie. Czy w ogóle to dodawać...}
